...
LOG_DIR=/log/game_fast/
SCRIPTS_DIR=/scripts/game_fast

source $SCRIPTS_DIR/error_handler_internal.sh $LOG_DIR main_game_fast_err.log --none

echo "main: Recieved Arguments...."
if bash $SCRIPTS_DIR/common_checks_internal.sh '--game-fast' $BUILD_TYPE $COMPONENT_ONLY_BUILDS $BUILD_CHANNEL $BUILD_TARGET; then
...
if [ $COMPONENT_ONLY_BUILDS == "--all" ]; then
  ...
  bash $SCRIPTS_DIR/build_game_fast_packages.sh $build_target $build_type $channel
fi
}
...
#Debug
build_game_fast --debug $LOCAL_BUILD_TYPE --dev

# Release Builds.
build_game_fast --release $LOCAL_BUILD_TYPE --dev
#----------------------------Dev Channel ends-----------------

#------------------------------------Stable Channel-----------"
#Debug
build_game_fast --debug $LOCAL_BUILD_TYPE --stable

# Release Builds.
build_game_fast --release $LOCAL_BUILD_TYPE --stable
#----------------------------stable Channel ends-----------------

echo "Built all packages needed for Game Fast!"
